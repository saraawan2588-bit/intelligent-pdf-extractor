...
class IntelligentExtractor:
    ...
    def _init_gemini(self, api_key: Optional[str] = None) -> None:
        """
        Initialize Google Gemini API with latest model
        ...
        # Use latest Gemini model (gemini-pro is deprecated)
        model_name = self.model if self.model != "auto" else "gemini-1.5-pro"
        ...
        try:
            ...
            logger.info("💡 Try updating: pip install --upgrade google-generativeai langchain-google-genai")
            raise
    ...
    def extract_with_ai(self, text: str, instructions: str) -> Dict:
        ...
        """
        logger.info(f"🤖 Using {self.provider.upper()} to extract data")
